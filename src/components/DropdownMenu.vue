...
const onKeydownToUpdateIndex = (newIndex: number, oldIndex: number) => {
  ...
  if (newIndex > oldIndex) {
    if (element.offsetTop + element.offsetHeight > dropdownMenu.value!
  .
    offsetHeight
  )
    {
    ...
    if (element.offsetTop < dropdownMenu.value!
  .
    scrollTop
  )
    {
...
class HandleHide {
  ...
  static onSelectionChange() {
    if (window.getSelection()?.rangeCount && window.getSelection()!
  .
    rangeCount > 0
  )
    {
